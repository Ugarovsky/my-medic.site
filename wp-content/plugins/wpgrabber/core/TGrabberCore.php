    ...
    */
    function getContent($url)
    {
        ...
        if (!$this->requestMethod) { // CURL
            ...
            if ($this->config->get('requestTime')) curl_setopt($ch, CURLOPT_TIMEOUT, $this->config->get('requestTime'));
            if ($this->config->get('proxyServ')) curl_setopt($ch, CURLOPT_PROXY, $this->config->get('proxyServ'));
            if ($this->config->get('proxyAuth')) curl_setopt($ch, CURLOPT_PROXYUSERPWD, $this->config->get('proxyAuth'));
            curl_setopt($ch, CURLOPT_RETURNTRANSFER, true);
            if ($this->config->get('curlRedirectOn')) curl_setopt($ch, CURLOPT_FOLLOWLOCATION, true); 
            curl_setopt($ch, CURLOPT_FAILONERROR, false);
            curl_setopt($ch, CURLOPT_USERAGENT, "Mozilla/5.0 (Windows NT 10.0; Win64; x64; rv:59.0) Gecko/20100101 Firefox/59.0");
            if ($this->config->get('userAgent')) curl_setopt($ch, CURLOPT_USERAGENT, $this->config->get('userAgent'));
            $out = curl_exec($ch);
            $this->currentUrl = $this->_rawurldecode(curl_getinfo($ch, CURLINFO_EFFECTIVE_URL));
            curl_close($ch);
    ...
    */
    private function _import()
    {
        ...
        elseif ($this->feed['type'] == 'vk') { // vk
            ...
            preg_match_all('|<div class="post_date"><a  class="post_link"  href="/(wall-\d+_\d+)".*?<div class="wall_text">(.*?)<div class="like_wrap|is', $index, $matches);
            if (!count($matches)) {
            ...
            foreach ($links as $link) {
                ...
                $this->content[$link]['text'] = preg_replace('~<div[^>]+class="(?:post_video_views|video_thumb_label|page_media_link|wall_marked|feedback|page_gif|page_post_queue_wide|wall_signed|page_media_poll).*?</div>~is', '', $this->content[$link]['text']);
              /*  $this->content[$link]['text'] = preg_replace('|<div class="page_post_queue_wide">.*?</div>|is', '', $this->content[$link]['text']);
				...
				$this->content[$link]['text'] = preg_replace('|<a href="/video[^>]+background-image:\s*url\(([^\)]+)[^>]+>|is', '<img src="$1" />', $this->content[$link]['text']);
        ...
        if ($this->feed['params']['filter_words_on']) {
            ...
            if ($this->feed['params']['filter_words_save']) { 
                if (count($_word_search[1])) {
                    $this->_echo('<br /><i>Материал <span style="color:red;">не будет сохранен</span> по причине наличия следующих фильтр-слов в нем: ' . implode(', ', $_word_search[1]) . '</i>');
                    return null;
                } else {
                    $this->_echo('<br /><i>Материал <span style="color:#1fbb1f;font-weight:bold;">будет сохранен</span> по причине отсутствия фильтр-слов в нем' . '</i>');
                }
            } elseif (!$this->feed['params']['filter_words_save']) {
                if (count($_word_search[1])) {
                    $this->_echo('<br /><i>Материал <span style="color:#1fbb1f;font-weight:bold;">будет сохранен</span> по причине наличия следующих фильтр-слов в нем: ' . implode(', ', $_word_search[1]) . '</i>');
                } else {
                    $this->_echo('<br /><i>Материал <b style="color:red;">не будет сохранен</b> по причине отсутствия фильтр-слов в нем' . '</i>');
                    return null;
                }
            } 
        }
...
*/        
        ...
        if ($this->imagesContentNoSave) {
            $this->_echo("<br>Материл <span style=\"color:red;\">не будет сохранен</span> по причине отсутсвия в нем картинок! (см. опцию: Не сохранять материал без картинок)");
            return null;
        }
        ...
        if (empty($record['text'])) {
          $this->_echo('<br /><i>Материл <span style="color:red;">не будет сохранен</span> по причине отсутствия в нем контента</i>');
          return null;
        }
        return true;
    }
