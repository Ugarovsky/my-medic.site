    ...
    function save($url)
    {
        ...
        if ($this->feed['params']['titleUniqueOn']) {
          if ($this->isTitle($record['title'])) {
            $this->_echo('<br><br /><b><span style="color:red;">Неуникальный заголовок<span>: "'.$record['title'].'" в заданной категории!</b>');
            return null;
          }
        }
        ...
        if ($postID) {
          if($this->saveContentRecord($postID, $url)) {
            $this->_echo("<br>Запись с заголовком: <b>{$record['title']}</b> - <span style=\"color:#00b700;\">успешно сохранена!</span>");
